(() => {
  ...
  const usageModeLabels={keyword:'뉴스·키워드',story:'메모·스토리'};
  ...
  const permissionFeatures=[['dashboard.extended','대시보드 (확장)','방송·시즌·주식 등 로그인 전용 데이터'],['trend.naver','네이버 검색어 트렌드','영역별 네이버 상대 검색지수 제공'],['studio.access','AI 콘텐츠 스튜디오','AI 글쓰기 화면 접근'],['ai.write','글쓰기','블로그 글과 쇼츠 생성 실행'],['ai.personalize','AI 개인화 설정','페르소나·시스템 지침 저장'],['billing.access','결제 및 쿠폰','결제·쿠폰 관련 기능 접근'],['admin.members','회원 관리','회원 역할·상태·쿠폰 관리'],['admin.permissions','권한 설정','관리자 전용 · 기능별 권한 변경'],['admin.service_plans','서비스 등급별 기능 설정','관리자 전용 · Free·Plus·Pro 제한과 제공 기능 관리'],['admin.billing_settings','결제 설정','관리자 전용 · 멤버십 결제와 커피 후원 관리'],['admin.system_settings','시스템 설정','관리자 전용 · AI 연결·뉴스 검색·공개 모델 관리']];
  const permissionAdminOnlyFeatures=new Set(['admin.permissions','admin.service_plans','admin.billing_settings','admin.system_settings']);
  ...
  function renderPermissions(){const body=$('permission-rows');body.innerHTML=permissionFeatures.map(([key,label,description])=>`<tr><td><strong>${label}</strong><span>${description}</span></td>${permissionRoles.map(role=>{const adminOnly=permissionAdminOnlyFeatures.has(key);const locked=role==='admin'||adminOnly;const checked=adminOnly?role==='admin':Boolean(permissionMatrix[role]?.[key]);return `<td><input class="permission-toggle" type="checkbox" data-role="${role}" data-feature="${key}" ${checked?'checked':''} ${locked?'disabled':''} aria-label="${label} ${role}"></td>`}).join('')}</tr>`).join('')}

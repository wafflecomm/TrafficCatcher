(() => {
  ...
  function renderMemberDetail(data){
    ...
    return `<div class="member-detail-grid"><section><h3>계정 세부 정보</h3><dl><div><dt>회원 ID</dt><dd>${escapeHtml(user.id||'-')}</dd></div><div><dt>이메일 인증</dt><dd>${user.email_verified_at?date(user.email_verified_at):'미인증'}</dd></div><div><dt>운영 역할</dt><dd>${escapeHtml(roleLabels[user.role]||user.role||'-')}</dd></div><div><dt>서비스 등급</dt><dd>${escapeHtml(planLabels[user.plan_code]||user.plan_code||'Free')}</dd></div><div><dt>이용 상태</dt><dd>${user.status==='active'?'정상':'이용 정지'}</dd></div><div><dt>가입일</dt><dd>${date(user.created_at)}</dd></div><div><dt>최근 로그인</dt><dd>${date(user.last_login_at)}</dd></div><div><dt>최근 정보 변경</dt><dd>${date(user.updated_at)}</dd></div></dl></section><section><h3>글쓰기 이용 현황</h3><div class="member-detail-metrics">${unlimited?'<div><span>현재 가능</span><strong>무제한</strong></div>':detailMetric('현재 가능',credits.balance,'건')}${detailMetric('누적 지급',credits.earned_total,'건')}${detailMetric('누적 사용',credits.used_total,'건')}${detailMetric('저장 원고',activity.draft_count,'건')}${detailMetric('전체 글쓰기',activity.writing_jobs,'건')}${detailMetric('완료',activity.completed_jobs,'건')}${detailMetric('실패',activity.failed_jobs,'건')}${detailMetric('이번 달 실행',activity.current_month_jobs,'건')}${detailMetric('이번 달 사용량',activity.current_month_units,'회')}${detailMetric('누적 사용량',activity.usage_units,'회')}${detailMetric('활성 로그인',activity.active_sessions,'개')}${detailMetric('추천 연결',activity.referral_count,'건')}</div><p class="member-credit-updated">글쓰기 건수 최종 변경: ${date(credits.updated_at)}</p></section></div><div class="member-history-grid"><section><h3>최근 글쓰기 실행 내역</h3><p class="member-history-privacy">입력 원문·완성 글·URL·API 키·IP는 저장하지 않으며 실행 메타데이터만 400일간 보관합니다.</p><ul class="member-job-list">${jobsHtml}</ul></section><section><h3>최근 관리자 변경 내역</h3><ul class="member-audit-list">${logsHtml}</ul></section></div>`;
  }
  ...
  let aiModels=[];
  let billingSettings={payment_enabled:false,donation_enabled:false},billingSettingsLoaded=false,billingSettingsLoading=null;
  ...
  function renderBillingSettings(){
    const payment=$('admin-payment-enabled'),donation=$('admin-donation-enabled');
  ...
  async function loadBillingSettings(force=false){
    ...
    billingSettingsLoading=(async()=>{try{const data=await api('/api/admin/billing-settings');billingSettings={payment_enabled:Boolean(data.settings?.payment_enabled),donation_enabled:Boolean(data.settings?.donation_enabled)};billingSettingsLoaded=true;renderBillingSettings();$('billing-settings-status').textContent=`현재 결제 설정을 불러왔습니다.${data.updated_at?` · 최종 변경 ${date(data.updated_at)}`:''}`;return data}catch(e){$('billing-settings-status').textContent=e.message;$('billing-settings-status').className='admin-status error';throw e}finally{sectionLoading('admin-billing-section',false);billingSettingsLoading=null}})();
    return billingSettingsLoading;
  }
  async function saveBillingSettings(){
    const button=$('save-billing-settings');const settings={payment_enabled:$('admin-payment-enabled').checked,donation_enabled:$('admin-donation-enabled').checked};button.disabled=true;
    try{const data=await api('/api/admin/billing-settings',{method:'PATCH',body:JSON.stringify(settings)});billingSettings=data.settings||settings;billingSettingsLoaded=true;renderBillingSettings();$('billing-settings-status').textContent=`저장 완료 · 결제 ${billingSettings.payment_enabled?'사용':'미사용'} · 후원 ${billingSettings.donation_enabled?'사용':'미사용'}`;$('billing-settings-status').className='admin-status'}catch(e){$('billing-settings-status').textContent=e.message;$('billing-settings-status').className='admin-status error'}finally{button.disabled=false}
  }

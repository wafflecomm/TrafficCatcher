...
절대 규칙과 지침 충돌 해결 규칙은 두 모드에 공통으로 읽되, 관리자가 `AI 최종 전달 순서`에서 해당 항목을 선택한 경우에만 최종 요청에 포함합니다.

관련 코드: `ai_studio_code.py`

## 8. 최종 `system_instruction` 조립 순서

최종 AI 시스템 지침은 다음 순서로 조립됩니다. 관리자는 상단 체크박스로 각 영역의 포함 여부를 전역 설정할 수 있으며, 일반 회원은 이 구성을 변경할 수 없습니다.
...
- 절대 규칙과 충돌 해결 규칙 → 관리자가 체크한 경우 포함
- 필수 출력물 형식 → 절대 규칙을 체크한 경우 읽기 전용 하위 규칙으로 함께 포함
- 페르소나·톤앤매너 → 관리자 체크와 사용자 페르소나 활성화 조건을 모두 만족할 때 포함
- 선택 글쓰기 유형 → 관리자가 체크한 경우 선택 모드의 개인 지침만 포함
- 관리자 구성 → `service_settings.ai_instruction_sections` JSON에 저장하고 감사 로그 기록
...
| 최종 지침 조립 | `ai_studio_code.py`가 서버 전역 설정 적용 | Worker가 D1 전역 설정 적용 |
...
- 절대 규칙과 충돌 해결 규칙은 관리자가 체크한 경우에만 포함합니다.
- 페르소나·톤앤매너는 관리자 체크와 사용자 활성화 조건을 모두 만족할 때 포함합니다.
- 최종 전달 체크 설정은 관리자만 변경하며 서버 DB와 감사 로그에 저장합니다.

...
`기본값 불러오기`를 누르면 메모·스토리용 템플릿을 편집기에 표시하며, 저장한 뒤부터 DB 개인 지침으로 사용합니다.

절대 규칙과 지침 충돌 해결 규칙은 두 모드에 공통으로 읽되, 현재 사용자가 `AI 최종 전달 순서`에서 해당 항목을 선택한 경우에만 최종 요청에 포함합니다.

관련 코드: `ai_studio_code.py`

## 8. 최종 `system_instruction` 조립 순서

최종 AI 시스템 지침은 다음 순서로 조립됩니다. `ai.personalize` 권한이 있는 로그인 사용자는 상단 체크박스로 자신의 요청에 포함할 영역을 선택할 수 있습니다. 체크 변경 시 별도 저장 버튼 없이 계정 DB에 즉시 저장됩니다.
...
선택 모드의 개인 지침만 2번 영역에 사용합니다. 기본 지침 파일은 사용자가 `기본값 불러오기` 후 DB에 저장하기 위한 템플릿이며 자동으로 대체 적용하지 않습니다.

- 개인 지침 있음 → 개인 지침 사용
- 개인 지침 없음 → 2번 영역을 빈 값으로 전달
- 절대 규칙과 충돌 해결 규칙 → 현재 사용자가 체크한 경우 포함
- 필수 출력물 형식 → 절대 규칙을 체크한 경우 읽기 전용 하위 규칙으로 함께 포함
- 페르소나·톤앤매너 → 현재 사용자 체크와 페르소나 활성화 조건을 모두 만족할 때 포함
- 선택 글쓰기 유형 → 현재 사용자가 체크한 경우 선택 모드의 개인 지침만 포함
- 개인 구성 → 사용자 ID별 `user_ai_instruction_sections`에 저장
...
| 최종 지침 조립 | `ai_studio_code.py`가 SQLite 사용자별 설정 적용 | Worker가 D1 사용자별 설정 적용 |
...
로컬 Python 생성기는 `article_mode`에 해당하는 DB 개인 지침만 2번 영역에 사용합니다. 개인 지침이 없으면 빈 값으로 전달하며 기본 지침 파일은 자동 적용하지 않습니다.

### 13.2 Cloudflare 운영 서버 경로

```text
D1에서 선택 모드 개인 지침 조회
  → 브라우저가 절대 규칙·선택 지침·페르소나·충돌 규칙을 분리 전달
  → Worker가 현재 사용자의 체크 설정을 D1에서 조회
  → 체크된 규칙만 system_instruction으로 조립
...
- 개인 지침이 없으면 선택 글쓰기 유형 지침을 빈 값으로 전달합니다.
- 기본 `.md` 지침은 `기본값 불러오기` 후 사용자가 DB에 저장한 경우에만 개인 지침으로 사용합니다.
- 절대 규칙과 충돌 해결 규칙은 현재 사용자가 체크한 경우에만 포함합니다.
- 페르소나·톤앤매너는 현재 사용자 체크와 페르소나 활성화 조건을 모두 만족할 때 포함합니다.
- 최종 전달 체크 설정은 `ai.personalize` 권한이 있는 사용자가 변경하며 사용자 ID별 서버 DB에 즉시 저장합니다.

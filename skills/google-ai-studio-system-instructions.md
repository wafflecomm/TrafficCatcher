...
- **뉴스 정확도 보완 및 유튜브 심층 분석 우선 적용**: 수집된 뉴스 기사의 정보가 단편적이거나 정확도/신뢰성이 떨어지는 경우, 함께 수집된 **유튜브 동영상의 실제 자막(Transcript), 영상 설명, 전문가 심층 분석 팩트를 최우선으로 정밀 분석하여 원고의 깊이와 체류시간을 극대화**합니다.
- **1개 출처만 제공된 경우**: 해당 출처의 오프닝 문제 제기(1장), 본론 상세 데이터 및 시연 분석(2장), 결론 및 실천 가이드(3장)로 3단계 심층 분할하여 1,500자 이상의 꽉 찬 원고로 전개합니다.
- **2~3개 출처가 제공된 경우**: 뉴스 보도 팩트와 유튜브 영상 분석 시각을 유기적으로 교차 대조하여 다각도의 심층 기사로 융합합니다.
- **새로운 기사로 재탄생**: 말실수나 불필요한 추임새를 제거하고, 매끄러운 저널리즘 및 친근한 블로그 포스팅으로 완벽히 재구성합니다.
- **참고한 출처 링크(URL) 표시**: 실제로 수집된 기사/영상 링크와 매체명/채널명을 본문 상단 또는 출처란에 정확히 명시합니다.
...
- 📌 **출처 2**: [실제 기사/영상 제목](실제 URL) (실제 언론사/유튜브 채널명)
*(반드시 프롬프트에 입력된 실제 수집 출처의 [제목], [URL 링크], [언론사/채널명]을 그대로 사용하여 클릭 가능한 링크 `[제목](URL)` 형식으로 작성해야 합니다)*

...
3.  **🤖 AI 블로그 포스팅 원고 & 쇼츠 4컷(9:16) 삽화 프롬프트 원스톱 생성 스튜디오 (블로그 수익화 & SEO 마스터)**
    ...
    *   **🎥 유튜브(YouTube) 실시간 영상 검색 & 자막(Transcript/CC) 텍스트 수집 (YouTube to Article) [NEW]**: 검색 키워드로 유튜브에서 상위 관련 영상을 실시간 검색하고 실제 음성 자막(CC) 텍스트를 글자로 추출하여, AI가 영상의 핵심 발화와 팩트를 100% 학습한 뒤 **파워블로거 스타일의 완성도 높은 블로그 기사**로 자동 창작.
    *   **💖 인플루언서 톤앤매너 & 1,500자~2,000자 이상 고밀도 분량**: `"이웃님들, 반가워요! 💖"` 통통 튀는 구어체와 풍부한 이모지(Emoji), 영상 오프닝 쟁점부터 상세 시연 및 유튜버 결론 제언까지 완벽 정리.
...
3.  **🔑 Google AI Studio (Gemini API) 연동 (선택사항)**:
    *   [Google AI Studio](https://aistudio.google.com/app/apikey)에서 무료 API Key를 발급받아 모달 내 **`🔑 Google AI Studio 연동 설정`**에 입력 후 저장합니다.
    *   API Key가 등록되면 실제 최신 **Gemini 1.5/2.0 Flash** 모델이 실시간으로 뉴스를 읽고 1,500~2,000자 이상의 고품질 원고를 직접 창작합니다. (키는 브라우저 `localStorage`에만 안전 보관)
    *   API Key가 없어도 **무료 내장 엔진**으로 0초 만에 안정적으로 생성됩니다.
4.  **원스톱 복사 및 활용 (마크다운 & HTML 2개 양식 지원)**:

...
* **프리미엄 모델 장시간 요청 안정화**: `gemini-3.7-flash`, `gemini-3.1-pro-preview`는 Interactions API `v1beta` 백그라운드 실행으로 작업 ID를 먼저 받은 뒤 3초 간격으로 완료 상태를 조회합니다. Cloudflare의 장시간 응답 대기로 인한 HTTP 524를 피하면서 사용자가 선택한 모델을 그대로 사용합니다.
...
* `gemini-3.7-flash`, `gemini-3.1-pro-preview`는 `background=true`, `store=true`로 시작하고 `/api/gemini/interactions/{id}`를 통해 완료 상태를 조회합니다.
...
* **HTTP 524 또는 응답 지연**: 브라우저를 강력 새로고침해 최신 Worker를 사용하고, 운영 빌드 정보와 `ai_background_jobs` 등록 여부를 확인합니다. 프리미엄 모델은 완료까지 수 분이 걸릴 수 있지만 작업 ID 등록 후에는 상태 조회 방식으로 처리됩니다.

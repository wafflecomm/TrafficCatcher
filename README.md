...
3.  **🤖 AI 블로그 포스팅 원고 & 쇼츠 4컷(9:16) 삽화 프롬프트 원스톱 생성 스튜디오 (실제 기사 팩트 반영)**
    *   **Role & Objective**: 네이버 블로그 SEO 검색엔진 최적화 및 숏폼(쇼츠/릴스) 알고리즘을 완벽 마스터한 수석 마케팅 AI 에이전트 엔진 탑재.
    *   **📰 실제 기사 내용 및 발언 인용구 반영**: 6하원칙에 입각한 실제 사건 발생 경위, 타임라인, 관계자 핵심 발언 인용문(`> 📌 주요 보도 인용`) 및 팩트가 풍부하게 포함된 실사형 기사체 원고 자동 작성.
    *   **[Step 1] 키워드 속성 판별**: 이슈/트렌드형, 정보/스테디형, 리뷰/상업형 서사 템플릿 자동 분류.
    *   **[Step 2] 3개 관점 교차 분석 표**: 신뢰성 높은 다각도 팩트 분석 및 객관적 비교 대조 표(Table) 자동 생성.
    *   **[Step 3] 네이버 블로그 복사-붙여넣기 템플릿**: 클릭을 부르는 추천 제목 3선, 3초 팩트 요약, H1~H3 구조화 본문, 핵심 데이터 표, 에디터 공감 코멘트, 추천 태그 8~10개 일괄 출력.
    *   **[Step 4] 9:16 세로형 쇼츠/릴스 연동 4컷 삽화 프롬프트**: 일관된 아트 스타일을 바탕으로 1컷(뉴스 속보 훅 0~2초), 2컷(사건 경위 팩트 3~5초), 3컷(핵심 해설 6~8초), 4컷(피날레/CTA 9~12초) 한글 및 Imagen 3 영문 프롬프트 생성.
4.  **연관 금융 정보 수집**
...
3.  **원스톱 복사 및 활용**:
    *   **📝 네이버 블로그 탭**: 제목 3선, 3줄 요약, 구조화 본문, 비교 표, 추천 태그가 포함된 원고를 **`📋 네이버 블로그 원고 전체 복사`** 버튼으로 즉시 복사하여 블로그 에디터에 붙여넣기합니다.
    *   **🎬 쇼츠/릴스 4컷 탭**: 9:16 세로형 4컷 스토리보드(훅, 문제제기, 해결책, CTA)를 확인하고 **`📋 Imagen 3 프롬프트 복사`**를 눌러 이미지 생성 AI에 입력합니다.

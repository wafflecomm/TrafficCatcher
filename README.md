...
* **키워드·뉴스 / 스토리·원고 작성 모드**: 실시간 검색 키워드 글과 사용자가 입력한 긴 스토리 기반 글을 분리해 작성합니다. 뉴스 검색 중 스토리 탭으로 이동해도 요청을 유지하고 키워드 탭으로 돌아오면 진행 화면 또는 완료 결과를 복원합니다.
...
* **AI 글쓰기 개인화**: 작성 카테고리, 페르소나, 말투 강도, 글의 밀도와 사용자 추가 요청을 계정별로 저장합니다. AI 시스템 지침 관리에서는 키워드·뉴스와 스토리·원고용 개인 지침을 각각 DB에 저장합니다.
* **일관된 글쓰기 UI**: 기능명은 `글쓰기`로 표준화하고 실제 버튼은 아이콘·문구를 가로·세로 중앙 정렬합니다. `뉴스 검색 키워드`, `기사 주제 또는 제목`, 시즌 문화 일정 검색창은 동일한 회색 입력 배경을 사용하며 서식 미리보기는 전체 외곽선을 표시합니다. 저장 원고 목록은 PC 최대 `1440px`, 모바일 전체 폭으로 제공합니다.
...
* `내 원고함`은 로그인 계정별로 최근 수정순 최대 5개를 저장합니다. 로컬 개발 환경은 회원 SQLite DB, Cloudflare 운영 환경은 D1 `AUTH_DB`를 사용하므로 별도 로컬 도우미를 실행하지 않아도 원고함을 이용할 수 있습니다. 버튼 뱃지는 저장된 원고 개수만 표시하며, 동일 제목은 갱신하고 6번째 저장 시 확인 후 가장 오래된 원고를 교체합니다.

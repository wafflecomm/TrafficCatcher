...
5.  **하이브리드 2-Way 데이터 조회 최적화**
    *   대시보드가 실행 환경을 자동 감지하여, 로컬 Flask 웹앱 구동 시에는 실시간 비동기 `/api/trends` 스캔 백엔드와 연계되고, Cloudflare Pages 호스팅 환경에서는 깃허브 액션이 정적 배포한 `/trends.json` 캐시 데이터베이스를 즉시 불러오도록 설계되었습니다.

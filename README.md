...
- 운영 사이트: <https://trafficcatcher.ai/>
- Pages 기본 주소: <https://trafficcatcher.pages.dev/>
...
- **서비스 등급·가격 관리 확장**: Free·Plus·Pro의 판매 여부, 월·연간 가격, 글쓰기·저장·개인화 한도를 관리자가 설정합니다. 내 프로필은 같은 DB 설정으로 요금과 제공량을 비교합니다. 상세 기획: [Markdown](docs/SERVICE_PLAN_PRICING_AND_PERMISSIONS_PLAN.md) · [HTML](docs/SERVICE_PLAN_PRICING_AND_PERMISSIONS_PLAN.html)

* **관리자 전용 설정 권한 고정**: `서비스 등급별 기능 설정`, `결제 설정`, `시스템 설정`과 `기능별 권한 설정`은 관리자만 조회·변경할 수 있습니다. 권한 표에서 일반 회원·유료 회원·운영자는 해제된 잠금 상태로 표시하고, 화면과 로컬·운영 API에서 모두 관리자 권한을 다시 검사합니다.
* **글쓰기 용어 간소화**: 반복 노출되던 `AI 글쓰기`, `AI 글 보완하기`를 각각 `글쓰기`, `글 보완하기`로 정리했습니다. API 상태·시스템 지침·페르소나처럼 AI임을 구분해야 하는 명칭은 유지합니다.
* **운영 AI 요청 시간 초과 방지**: 운영 서버의 모든 글쓰기 모델은 Interactions API `v1beta` 백그라운드 실행으로 작업 ID를 먼저 받은 뒤 3초 간격으로 완료 상태를 조회합니다. Cloudflare의 장시간 응답 대기로 인한 HTTP 524를 피하면서 사용자가 선택한 모델을 그대로 사용합니다.
* **AI 제공업체 과부하 자동 재시도**: 선택 모델이 `high demand`, `service unavailable`, `503` 같은 일시적 혼잡을 반환하면 같은 모델로 8초 후 1차, 20초 후 2차 재시도합니다. 임의로 하위 모델로 변경하지 않으며 모두 실패하면 다른 모델 선택을 안내합니다.
...
* **뉴스·키워드 / 메모·스토리 작성 모드**: 실시간 검색 키워드 글과 사용자가 입력한 짧은 메모나 이야기를 분리해 작성합니다. 뉴스 검색 중 메모·스토리 탭으로 이동해도 요청을 유지하고 키워드 탭으로 돌아오면 진행 화면 또는 완료 결과를 복원합니다.
* **시스템 지침 분리 관리**: 뉴스·키워드와 메모·스토리는 작성 방식에 맞는 별도의 내부 시스템 지침을 사용합니다. 로컬 서버는 요청 시 최신 지침을 다시 읽고, 클라우드는 배포된 최신 지침을 불러옵니다.
...
* **글쓰기 개인화**: 작성 카테고리, 페르소나, 말투 강도, 글의 밀도와 사용자 추가 요청을 계정별로 저장합니다. AI 시스템 지침 관리에서는 뉴스·키워드와 메모·스토리용 개인 지침을 각각 DB에 저장합니다.
...
* **1+ 적응형 팩트 합성 (1+ Adaptive Synthesis)**:
  * 실시간 구글 뉴스 및 유튜브 영상에서 수집된 팩트 출처가 **1개만 있어도, 2개 또는 3개가 있어도** 1,500자~2,000자 이상의 꽉 찬 고품질 글로 자동 재구성.
...
* 글쓰기용 AI API 키는 Cloudflare의 암호화 Secret `GEMINI_API_KEY`에서만 관리하며 브라우저에 입력·저장하거나 외부 AI 서비스로 직접 노출하지 않습니다.
...
* 운영 Worker는 AI Interactions API `v1beta`와 `Api-Revision: 2026-05-20` 헤더를 사용합니다. `thinking_level`은 지원 모델 공통값인 `low`로 전달합니다.
* 운영 서버의 모든 글쓰기 모델은 `background=true`, `store=true`로 시작하고 `/api/gemini/interactions/{id}`를 통해 완료 상태를 조회합니다.
* 백그라운드 작업 ID는 로그인 사용자와 연결해 D1 `ai_background_jobs`에 저장하므로 다른 사용자의 작업을 조회할 수 없습니다.
* 일반 회원 작업이 API 실패로 종료되면 예약한 글쓰기 쿠폰은 중복 환불되지 않도록 상태를 확인한 뒤 복원합니다. 관리자·운영자·프리미엄 권한은 무제한 정책을 따릅니다.
* 모델 혼잡 자동 재시도 후에도 실패하면 선택 모델을 유지한 채 오류를 안내합니다. 빠른 작성이 필요하면 `Flash 3.6` 또는 `Flash 3.5`를 직접 선택할 수 있습니다.

### 8. ✨ AI 기반 전체 글 보완

* `글 보완하기`는 입력 문장을 하단에 붙이지 않고 현재 글 전체와 보완 요청을 AI가 다시 분석합니다.
...
* `미완의 글서랍`은 서비스 등급별 저장 한도 안에서 원고를 보관합니다. 로컬 개발 환경은 회원 SQLite DB, Cloudflare 운영 환경은 D1 `AUTH_DB`를 사용하며 동일 제목은 기존 원고로 갱신합니다.
* 관리자가 저장 한도를 기존 저장 개수보다 낮춰도 원고·시스템 지침서·페르소나는 삭제되지 않습니다. 기존 항목의 조회·수정·적용·삭제는 유지되고 신규 저장과 복제만 차단됩니다.
...
#### 개인정보 최소화 사용내역과 유료회원 관리

* 글쓰기·글 보완 실행 시 `작업 종류`, `키워드/스토리 모드`, `AI 모델`, `성공/실패 상태`, `직접/중계/백그라운드 실행 환경`, `자료 유형`, `입력·출력 글자 수`, `처리 시간`, `사용량`, `쿠폰 차감·환불 여부`, `안전한 오류 분류 코드`, `실행 시각`만 기록합니다.
* 검색 키워드, 뉴스·YouTube 제목과 URL, 사용자가 입력한 스토리·요청문, 시스템 지침, AI 완성 글, API 키, IP 주소, 브라우저 정보는 사용내역 테이블에 저장하지 않습니다.
* 사용내역은 생성 시점부터 400일간 보관한 뒤 로컬 SQLite와 Cloudflare D1에서 자동 삭제합니다. 기존 백그라운드 작업은 본문 없이 메타데이터로 한 번만 이관됩니다.
* 관리자는 회원 목록의 행을 펼쳐 전체·이번 달 실행 횟수와 사용량, 성공·실패, 처리 시간, 모델·실행 환경, 쿠폰 차감 여부를 확인하고 회원별 글쓰기 가능 건수를 조정할 수 있습니다.
* 유료회원은 역할과 쿠폰 잔액으로 이용 권한을 관리하며, 프리미엄·운영자·관리자는 무제한 정책을 사용하더라도 사용량은 1회 단위로 집계되어 서비스 비용과 품질을 확인할 수 있습니다.
...
최신 운영 소스와 문서를 새 컴퓨터로 받을 때는 다음 저장소의 `main` 브랜치를 사용합니다.

```bash
git clone https://github.com/wafflecomm/TrafficCatcher.git
cd TrafficCatcher
git pull --ff-only origin main
```

이미 저장소가 있다면 작업 파일을 먼저 커밋하거나 별도로 보관한 뒤 `git pull --ff-only origin main`을 실행합니다. `.env`, `.env.server.txt`, SSH 개인키와 브라우저 `localStorage` 값은 보안상 GitHub에 포함되지 않으므로 새 컴퓨터에 안전한 방법으로 별도 복사해야 합니다. 동일한 Cloudflare 프로젝트와 GitHub Repository Secret은 서버에 유지되므로 소스를 다시 받을 때 API 키를 Git 파일에 추가하지 않습니다.
...
6. **글쓰기**: Cross Trending의 글쓰기 버튼을 사용하거나 포털 키워드·방송 프로그램·시즌 황금 키워드·인기 주식의 데이터 행을 선택하고 확인창에서 승인합니다.
...
* **뉴스·키워드 시스템 지침**: 내부 운영 지침이 단일 원본이며 글쓰기 전에 캐시 없이 다시 로딩됩니다.
* **절대 규칙이 반영되지 않음**: 로컬 서버를 재시작하고 운영 지침 API가 정상 응답하는지 확인합니다. Cloudflare에서는 최신 운영 지침이 배포됐는지 확인합니다.
* **HTTP 524 또는 응답 지연**: 브라우저를 강력 새로고침해 최신 Worker를 사용하고, 운영 빌드 정보와 `ai_background_jobs` 등록 여부를 확인합니다. 모든 모델은 작업 ID 등록 후 상태 조회 방식으로 처리되며, 모델 혼잡 시 완료까지 수 분이 걸릴 수 있습니다.
* **선택 모델이 혼잡함**: `high demand` 메시지는 연결 실패가 아니라 AI 제공업체의 일시적 과부하입니다. 같은 모델로 최대 두 번 자동 재시도하며, 계속 실패하면 `Flash 3.6` 또는 `Flash 3.5`를 선택합니다.
